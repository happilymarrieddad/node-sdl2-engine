...
const SDL_ttf = require('sdl2-ttf').require('SDL_ttf')
...
class render {
	...
	drawFont(text,color,file = 'Sans.ttf',size = 12,x,y,w,h) {
		let _font = SDL_ttf.TTF_OpenFont(file,size)
		let _message = SDL_ttf.TTF_RenderText_Solid(_font,text,color)
		let _texture = SDL_render.SDL_CreateTextureFromSurface(this._render,_message)
		let _rect = SDL_rect.SDL_Rect
		_rect.x = x
		_rect.y = y
		_rect.w = w
		_rect.h = h
		SDL_render.SDL_RenderCopy(this._render,_texture,null,_rect)
	}
	present() {
